...
func (s *ConnectServiceHandler) ListSharedMemoComments(ctx context.Context, req *connect.Request[v1pb.ListSharedMemoCommentsRequest]) (*connect.Response[v1pb.ListSharedMemoCommentsResponse], error) {
	resp, err := s.APIV1Service.ListSharedMemoComments(ctx, req.Msg)
	if err != nil {
		return nil, convertGRPCError(err)
	}
	return connect.NewResponse(resp), nil
